...
export const StyledToaster = styled(Toaster)(({ theme }) => {
  ...
  const loadingStyles = {
    ...
    [`& .${toasterClasses.loadingIcon}`]: {
      ...
      background: `conic-gradient(${varAlpha(theme.vars.palette.text.primaryChannel, 0)}, ${varAlpha(
  ...
  return {
    ...
    [`& .${toasterClasses.icon}`]: {
      ...
      borderRadius: "inherit",
      justifyContent: "center",
      alignSelf: "center", // Zmień z flex-start na center, aby całe tło ikony było w pionie tekstu
      [`& .${toasterClasses.iconSvg}`]: {
        color: "inherit", // Opcjonalnie: użyj koloru zdefiniowanego w typie tosta
        width: 24,
        height: 24,
        fontSize: 0,
      },
